...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    ...
    {
        case 0:
            sectionName = NSLocalizedString(@"Upcoming Trips", @"Upcoming Trips");
            break;
        case 1:
            sectionName = NSLocalizedString(@"Current Trips", @"Current Trips");
            break;
        case 2:
            sectionName = NSLocalizedString(@"Completed Trips", @"Completed Trips");
            break;
        default:
            sectionName = @"";
            break;
...
- (NSInteger)tableView:(UITableView *)theTableView numberOfRowsInSection:(NSInteger)section
{
    if(section==0)
    {
        if(_arrUpcomingTrips.count>0)
            return [_arrUpcomingTrips count];
        else
            return 1;
    }
    else if(section==1)
    {
        if(_arrCurrentTrips.count>0)
            return [_arrCurrentTrips count];
        else
            return 1;
    }
    else if(section==2)
    {
        if(_arrCompletedTrips.count>0)
            return [_arrCompletedTrips count];
        else
            return 1;
    }
    return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(15,5,cell.frame.size.width-30,30)];
    messageLabel.text = @"No trip is available. Please pull down to refresh.";
    ...
    switch (indexPath.section)
    {
        case 0:
            if(_arrUpcomingTrips.count>0)
            {
                objTrip = [_arrUpcomingTrips objectAtIndex:indexPath.row];
            }
//            else{
//            }
            break;
        case 1:
            if(_arrCurrentTrips.count>0)
            {
                objTrip = [_arrCurrentTrips objectAtIndex:indexPath.row];
            }
//            else{
...
//            }
        ...
        case 2:
            if(_arrCompletedTrips.count>0)
            {
                objTrip = [_arrCompletedTrips objectAtIndex:indexPath.row];
            }
//            else{
//                messageLabel.text = @"No data is currently available. Please pull down to refresh.";
//                messageLabel.textColor = [UIColor blackColor];
...
//        x += 70;
    ...
    btnCoTraveller.frame = CGRectMake(195,110,26,26);
    
//    TripObject *obj = [_arrData objectAtIndex:indexPath.row];
//    arrCoTravellerDetails = [[NSMutableArray alloc]initWithArray:[obj valueForKey:@"co_travellers"]];
//    if(arrCoTravellerDetails.count>0)
//        btnCoTraveller.hidden = false;
//    else
//        btnCoTraveller.hidden = true;
    
    UIButton *btnShareTrip = [UIButton buttonWithType:UIButtonTypeCustom];
    [btnShareTrip setImage:[UIImage imageNamed:@"share_orange"] forState:UIControlStateNormal];
    [btnShareTrip addTarget:self action:@selector(pressShareTripBtnTapped:) forControlEvents:UIControlEventTouchUpInside];
    btnShareTrip.tag = indexPath.row;
    btnShareTrip.tintColor = COLOR_ORANGE;
    btnShareTrip.frame = CGRectMake(150,110,26,26);
...
//    cell.layer.borderColor = [UIColor orangeColor].CGColor;
    ...
    [viewBG addSubview:btnShareTrip];
...
- (IBAction)pressShareTripBtnTapped:(id)sender
{
    ...
    [actionSheet addAction:[UIAlertAction actionWithTitle:@"Share with Member" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                            {
                                UIAlertController * alertController = [UIAlertController alertControllerWithTitle: @""
                                                                                                          message: @"Share this trip with your travel companion"
                                ...
                                [alertController addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                                                            {
                                                                ...
                                                                [self wsShareTripWithMembers:namefield.text :[obj valueForKey:@"id"]];
                                                            }]];
    ...
    [actionSheet addAction:[UIAlertAction actionWithTitle:@"Share via Email" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                            {

                            }]];
    [actionSheet addAction:[UIAlertAction actionWithTitle:@"Share via Facebook" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                            {
                                
                            }]];
    
    [actionSheet addAction:[UIAlertAction actionWithTitle:@"Share via Twitter" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
                            {
...
- (IBAction)pressShowCoTravellerDetails:(id)sender
{
    ...
    TripObject *obj = [_arrData objectAtIndex:i];
    arrCoTravellerDetails = [[NSMutableArray alloc]initWithArray:[obj valueForKey:@"co_travellers"]];
    [self performSegueWithIdentifier:@"segueMyTripCoTraveller" sender:self];
}
...
-(void)loadTripDetails //: (int)noPage
{
    @try{
        ...
        WSFrameWork *wsLogin = [[WSFrameWork alloc]initWithURLAndParams:WS_TRIP_LIST dicParams:Parameters];
        ...
        wsLogin.onSuccess=^(NSDictionary *dicResponce)
        {
           ...
           // HIDE_AI;
            ...
            else
            {
                ...
                _arrUpcomingTrips = [arr valueForKey:@"upcommingtrips"];
                _arrCurrentTrips = [arr valueForKey:@"currenttrips"];
                _arrCompletedTrips = [arr valueForKey:@"completedtrips"];
                
                [_arrData addObjectsFromArray:_arrUpcomingTrips];
                [_arrData addObjectsFromArray:_arrCurrentTrips];
                [_arrData addObjectsFromArray:_arrCompletedTrips];
        ...
        wsLogin.onSuccess=^(NSDictionary *dicResponce)
        {
            ...
            else
            {
                ...
                if([[dictData valueForKey:@"trip_type"]intValue]==1||[[dictData valueForKey:@"trip_type"]intValue]==2)
                {
                    ...
                    [self performSegueWithIdentifier:@"segueEditTripCitySwipe" sender:self];
                }
...
-(void)wsShareTripWithMembers :(NSString*)strEmailAddress :(NSString*)trip_id
{
    @try{
        SHOW_AI;
        NSDictionary *Parameters = [NSDictionary dictionaryWithObjectsAndKeys:
        ...
        WSFrameWork *wsLogin = [[WSFrameWork alloc]initWithURLAndParams:WS_SHARE_TRIP_WITH_MEMBER dicParams:Parameters];
        wsLogin.isLogging=TRUE;
        wsLogin.isSync=TRUE;
        wsLogin.onSuccess=^(NSDictionary *dicResponce)
        {
...
{
    if([[segue identifier] isEqualToString:@"segueEditTripCitySwipe"])
    {
    ...
    if([[segue identifier] isEqualToString:@"segueMyTripCoTraveller"])
    {
