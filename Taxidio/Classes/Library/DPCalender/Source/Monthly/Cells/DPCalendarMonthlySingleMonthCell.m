...
-(void)drawRect:(CGRect)rect {
    ...
    for (DPCalendarEvent *event in self.events) {
        ...
        if (self.eventDrawingStyle == DPCalendarMonthlyViewEventDrawingStyleBar) {
            //Draw Bar
            [self drawCellWithColor:[color colorWithAlphaComponent:1.0] InRect:CGRectMake(startPosition, event.rowIndex * self.rowHeight + ROW_MARGIN, width, self.rowHeight - ROW_MARGIN) context:context];
        } else {
            //Draw Underline
