...
- (void)goToCoachMarkIndexed:(NSUInteger)index {
    ...
    {
        btnSkipCoach = [[UIButton alloc] initWithFrame:(CGRect){{0, [self yOriginForContinueLabel]}, {self.frame.size.width, 30}}];
...
//        btnSkipCoach = [[UIButton alloc] initWithFrame:(CGRect){{lblContinueWidth, [self yOriginForContinueLabel]}, {btnSkipWidth, 30.0f}}];
        ...
        btnSkipCoach.titleLabel.font = [UIFont boldSystemFontOfSize:20.0];
        btnSkipCoach.alpha = 0.0f;
        [btnSkipCoach setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        
        //btnSkipCoach.backgroundColor
        [self addSubview:btnSkipCoach];
        [UIView animateWithDuration:0.3f delay:1.0f options:0 animations:^{
            btnSkipCoach.alpha = 1.0f;
        } completion:nil];
    }
    btnSkipCoach.hidden = FALSE;
